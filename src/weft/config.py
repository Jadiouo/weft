...
class S3Config(StageConfig):
    ...
    講者對著同一張圖講完兩件事時，它一刀都不會切。票 07 實測保留集召回
    只有 0.60。見 `experiments/r30_segmentation/REPORT.md`。
    """

    min_segment_sec: float = 5.0
    #: 分段方法。`ngram` 是票 07 的量測結果選出來的。
    #:
    #: 保留集 ±20 秒 F1：投影片切換驅動 0.439、切換降為候選 0.471–0.529、
    #: 純語意 0.667。
    #:
    #: **但 ±20 秒那個容忍窗量不出東西**（R37）——它覆蓋 80–95% 的時間軸。
    #: 改在 ±10 秒量、並補上「不看內容等距切」的對照組之後，
    #: `ngram` 與 Sentence-BERT **不是打平，是互有勝負**：
    #: 講經保留集 0.400 vs MiniLM 0.476，STEM 保留集則是 ngram 對等距
    #: 優勢最大（+0.30）。三支素材扛不住換方法的重量。
    #: **留著 `ngram` 是因為它不需要模型、GPU、下載**——與 §2.3 的純本地一致。
    #:
    #: `slide` 保留是為了能重現 v0.4 的行為做對照，**不是**建議值。
    method: str = "ngram"
    #: TextTiling 的偽句長度（字）。**未校準**——票 07 掃過 20/30/40/60/80/120，
    #: F1 在 0.353–0.762 之間跳，而 ngram 與 sbert 的排名隨它翻來翻去。
    #: 40 是「調校集選參數、保留集報分數」協定下 sbert 的最佳值；
    #: ngram 在調校集的最佳值是 60。取 40 因為它在兩支上都不是最差，
    #: **不是因為它最好**。（那次掃描同樣是在 ±20 秒下做的，見 R37——
    #: **這個參數等於沒有量過**。）
    block_chars: int = 40
