...
#: S4c 產出量的**災難下限**：每 1000 字逐字稿至少要產出幾個字。
#:
#: 用壓縮比而不是「字/分」——後者是速率，短素材分母太小就沒意義
#: （實測 90 秒的合成 fixture 只有 19.3 字/分，真實影片是 70–281）。
#:
#: 不是品質門檻。實測八支落在 **362–888**，各素材差很多。
#: 它偵測的是「S4c 幾乎沒回東西」。
#:
#: **為什麼需要它**：2026-08-09 把 `depth_alpha` 從 −0.5 調到 +0.75，
#: 分段品質大幅改善（WindowDiff 0.490 → 0.359），但 `2FjApOVIbUs` 的
#: 總產出字數**掉了 47%**，而溯源通過率反而從 0.979 升到 **1.000**——
#: 分母變小，所有既有指標都說變好了。
#:
#: 機制：**S4c 每段產出的 block 數與段落長短無關**（實測 1.2–1.9 個），
#: prompt 裡沒有任何規定。段數砍半 → 內容砍半。
#: D31 的 `_MAX_BARREN_RATIO` 是同一個形狀的較粗版本，它擋「整段空白」，
#: 擋不住「每段都少寫一半」。
#:
#: 100 取在實測最低值（362）的四分之一附近，**刻意寬鬆**：它要抓的是崩潰，
#: 不是退步。退步靠 `provenance.jsonl` 裡的 `chars_per_1k_source` 趨勢看。
#:
#: 實測的兩端：`2FjApOVIbUs` 調整 α 之後 362（八支最低，而它調整前是 680），
#: `cvb4Bl93lzQ` 888（純口播無投影片）。
MIN_CHARS_PER_1K_SOURCE: Final[float] = 100.0
