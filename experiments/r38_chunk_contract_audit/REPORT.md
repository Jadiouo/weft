...
> **兩處後來變了，2026-08-13 補正：**
>
> 1. **第二層已移除**（D34）——未過閘門的影片不再整支排除，
>    八支全收，354 筆。
> 2. **「完全相等」只在沒有超長 block 時成立。** §4.9 規定超過
>    `max_chunk_chars`（800）的 block 按句切分，一個 block 會產生多個 chunk。
>    實測 `UiKi5-Arce4#026#b00` 正好 800 字，切成兩片，
>    所以那支是 56 chunk 對 55 verified。
>    正確的契約是 **chunk 數 ≥ verified 數**，且**每一筆都通過溯源**。
