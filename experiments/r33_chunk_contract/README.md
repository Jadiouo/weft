...
chunk 文字中位長度 57 字、最長 269 字。

> **這個範圍夠不夠用是假設，未驗證。** `.raw/` 收件匣期待的長度沒有
> 任何地方定義過（那是 vault 的事，本 repo 觀察不到）。列在這裡是為了
> 讓 vault 側有數字可以對，不是為了聲稱它合格。

## 2. 三個缺口

### 2.1 `series_id` 與 `episode_index` 全部是 `null`

69/69 都是 null。原因是這兩支是**逐支抓的**，不是從 playlist 展開的，
